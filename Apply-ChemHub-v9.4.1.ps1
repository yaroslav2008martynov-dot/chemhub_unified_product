...
Write-Host "ChemHub v9.4.1 extractor + site rendering safe patch" -ForegroundColor Cyan

$root = Split-Path -Parent $MyInvocation.MyCommand.Path
Set-Location $root

if (-not (Test-Path ".\patch_files\apps\api\app\extractor.py")) {
  Write-Host "ERROR: patch_files not found. Extract the ZIP into C:\chemhub_unified_product first." -ForegroundColor Red
  exit 1
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backup = Join-Path $root "backup_before_v9_4_1_$stamp"
...
Write-Host "Applying safe DB compatibility and cleanup..."
...
# Safe cleanup: no PostgreSQL regex, only simple ILIKE filters.
...
$smokePath = Join-Path $root "chemhub_v9_4_1_smoke.py"
$smokeLines = @(
  ...
  "    print('CASE', c)",
  "    print([(getattr(r,'equation',''), getattr(r,'temperature',''), getattr(r,'conditions',''), getattr(r,'catalysts',''), getattr(r,'reaction_name','')) for r in rs])",
...
docker compose cp $smokePath "api:/tmp/chemhub_v9_4_1_smoke.py" | Out-Host
try {
  docker compose exec -T api sh -lc "cd /app && python /tmp/chemhub_v9_4_1_smoke.py" | Out-Host
} catch {
  Write-Host "Smoke test failed. Check logs: docker compose logs api --tail=80" -ForegroundColor Yellow
  throw
}

Write-Host "Patch v9.4.1 installed. Open site and AI Agent, then press Ctrl+F5." -ForegroundColor Green
