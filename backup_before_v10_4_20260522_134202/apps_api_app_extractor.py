...
GROUP_ALIASES = {
    ...
    "элементы 13 группы": GROUP_13, "элементы 14 группы": GROUP_14, "элементы 15 группы": GROUP_15, "элементы 16 группы": GROUP_16, "элементы 17 группы": GROUP_17,
...
BAD_CONTEXT_RE = re.compile(
    r"\b(катод|анод|электрон|полуреакц|электронн|баланс|pka|pkb|рка|ркб|пка|пкб|пкa|пкb|пр\s*=|ks\s*=|kb\s*=|ka\s*=)\b|[ē]",
    re.I,
)
...
NAME_WORDS = ["синтез рашиг", "метод байер", "габер", "бош", "магическая кислота", "основание миллона", "тефлон", "синтез-газ"]
...
def _clean_spaces(text: str) -> str:
    text = str(text or "")
    text = text.replace("\ufeff", "")
    # common arrows and mojibake arrow leftovers
    text = text.replace("⟶", "→").replace("⟹", "→").replace("=>", "→").replace("->", "→")
    text = text.replace("<=>", "⇌").replace("<->", "⇌").replace("↔", "⇌").replace("⇄", "⇌")
    text = text.replace("в†’", "→").replace("в‡Њ", "⇌").replace("в†‘", "↑").replace("в†“", "↓").replace("в‰ ", "≠")
...
def strip_oxidation_and_charges(text: str) -> str:
    """Strictly remove oxidation states and explicit charges from equations.

    User preference now: no superscript/charge information in saved equations.
    Formula indices and stoichiometric coefficients are kept.
    """
    text = _clean_spaces(_translate_lookalikes(text))
    # H20 can be OCR for H2^0 at formula boundary, but H20 in H2O context remains water.
    text = re.sub(r"\bH20(?=\s*(?:[+→⇌]|$))", "H2", text)
    text = text.replace("H₂O", "H2O").replace("H₂", "H2").replace("H20", "H2O")
    # remove charges/oxidation inside brackets as well: [S8^2+] -> [S8], [AsF6-] -> [AsF6]
    text = re.sub(r"(\[[^\]]*?)(?:\^\s*\d*[+-]|[+-]\s*\d*|[⁺⁻])(?=[^\]]*\])", r"\1", text)
    # remove attached oxidation states outside brackets: H2^0, Ca0, Al+3, H-1
    ...
    # remove isolated ion plus/minus after formulas in equations
    text = re.sub(r"\b([A-Z][A-Za-z0-9()\[\]]*)\s*([+-])(?=\s*(?:[+→⇌]|$))", r"\1", text)
    return _clean_spaces(text)

def fix_ocr_formula(text: str) -> str:
    text = strip_oxidation_and_charges(text)
...
def normalize_condition(cond: str) -> dict:
    ...
    # Preserve everything visibly above the arrow as condition too.
    ...
    for key, values in GROUP_ALIASES.items():
        if key in low:
            if "гал" in key or "hal" in key:
                defs["X"] = values
                defs["Hal"] = values
            elif "халькоген" in key or "chalcogen" in key or "пниктоген" in key or "pnictogen" in key:
                defs.setdefault("E", values)
            else:
...
def _replace_template_var(cur: str, var: str, val: str) -> str:
    # Replace template variables both standalone and inside formula chunks: 2MH -> 2NaH, KX -> KCl.
    if var == "Hal":
        return re.sub(r"(?<![A-Za-zА-Яа-я])Hal(?![a-zа-я])", val, cur)
    if var == "Me":
        return re.sub(r"(?<![A-Za-zА-Яа-я])Me(?![a-zа-я])", val, cur)
    return re.sub(rf"{re.escape(var)}", val, cur)
...
def expand_templates(eq: str, context: str) -> list[str]:
    ...
    for v in ["Hal", "Me", "M", "X", "E", "Э"]:
        if re.search(rf"(?<![A-Za-zА-Яа-я]){re.escape(v)}(?![a-zа-я])", eq) or (v in ["M", "X", "E"] and v in eq):
            if v in defs:
                vars_found.append(v)
