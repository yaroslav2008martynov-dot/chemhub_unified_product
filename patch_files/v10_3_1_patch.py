...
text = re.sub(
    r'\n@app\.(?:delete|post|get)\("/admin/reactions/by-source-pdf"[\s\S]*?(?=\n@app\.|\ndef |\nclass |\Z)',
    "\n",
    text,
)
...
marker = '@app.delete("/admin/reactions/{reaction_id}"'
if 'def admin_delete_by_source_pdf' not in text:
    if marker in text:
...
admin_js = """import React, { useEffect, useMemo, useRef, useState } from 'react';
...
"""
Path("apps/admin/src/main.jsx").write_text(admin_js, encoding="utf-8")
