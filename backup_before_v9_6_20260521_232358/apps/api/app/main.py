...
@app.get("/sitemap.xml")
def public_sitemap(db: Session = Depends(get_db)):
    return seo_sitemap(db)
...
def _info_score(obj) -> int:
    return len(obj.equation or "") + 8 * bool(obj.temperature) + 8 * bool(obj.conditions) + 8 * bool(obj.catalysts) + 8 * bool(obj.solvents) + 12 * bool(obj.reaction_name)


def _apply_jr_to_reaction(target: Reaction, jr: JobReaction) -> None:
    for key in ["reaction_name", "equation", "canonical_equation", "reactants", "products", "conditions", "catalysts", "solvents", "temperature", "pressure", "states", "source_pdf", "source_page", "confidence_score", "internet_status", "internet_note"]:
        if hasattr(target, key) and hasattr(jr, key):
            setattr(target, key, getattr(jr, key))
...
def publish_reactions(reaction_ids: list[int], db: Session = Depends(get_db)):
    ...
    for reaction_id in reaction_ids:
        ...
        if not jr or jr.published or not getattr(jr, "selected", True):
            continue
        key = canonical_equation(jr.equation)
        jr.canonical_equation = key
        hidden = getattr(jr, "review_reason", "") == "does_not_react" or "≠" in (jr.equation or "")
        existing = db.query(Reaction).filter(Reaction.canonical_equation == key).first()
        if existing:
            if _info_score(jr) > _info_score(existing):
                _apply_jr_to_reaction(existing, jr)
        ...
        reaction = Reaction(
            ...
            validation_status="does_not_react" if hidden else "approved_from_agent",
            impossible_note="не реагируют между собой" if hidden else "",
            approved=True,
            hidden=hidden,
            origin="agent",
        )
...
def _query_variants(q: str) -> set[str]:
    raw = (q or "").strip()
    variants = {raw, raw.replace(" ", "")}
    low = raw.lower().replace(" ", "")
    typo = low.replace("0", "o")
    typo = typo.replace("hl", "hcl").replace("naoh", "NaOH").replace("hcl", "HCl")
    variants.add(typo)
    try:
        variants.add(canonical_equation(raw))
        variants.add(canonical_equation(typo))
    except Exception:
        pass
...
def list_reactions(q: str = "", include_hidden: bool = False, db: Session = Depends(get_db)):
    ...
    if q_clean:
        if "/reaction/" in q_clean or q_clean.startswith("reaction/"):
            ...
            filters = []
            for v in _query_variants(q_clean):
                like = f"%{v}%"
                filters.extend([
                    ...
                    Reaction.states.ilike(like), Reaction.canonical_equation.ilike(like), Reaction.impossible_note.ilike(like),
                ])
...
def cleanup_duplicates(db: Session) -> None:
    ...
    for r in reactions:
        new_key = canonical_equation(r.equation or "")
        if r.canonical_equation != new_key:
            r.canonical_equation = new_key
    db.commit()
    groups: dict[str, list[Reaction]] = {}
    for r in db.query(Reaction).all():
        if r.canonical_equation:
            groups.setdefault(r.canonical_equation, []).append(r)
    for items in groups.values():
        if len(items) <= 1:
            continue
        best = max(items, key=_info_score)
        for r in items:
            if r.id != best.id:
                r.hidden = True
                r.approved = False
