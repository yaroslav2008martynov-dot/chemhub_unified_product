...
def _dict_to_reaction_obj(d: dict):
    ...
    r.impossible_note = d.get("impossible_note", "") or ""
    r.hidden = bool(d.get("hidden", False))
    return r


def _info_score(r) -> int:
...
def _dedup_reactions(items):
    best = {}
    for r in items:
        key = canonical_equation(getattr(r, "equation", ""))
        if not key:
            continue
        if key not in best or _info_score(r) > _info_score(best[key]):
            best[key] = r
...
def _process_pdf_job_sync(job_id: int, file_path: str, filename: str) -> None:
    ...
    try:
        ...
        for page_index, page in enumerate(doc, start=1):
            ...
            for reaction in _dedup_reactions(found):
                key = canonical_equation(reaction.equation)
                if key in seen_global:
                    continue
                seen_global.add(key)
                impossible = bool(getattr(reaction, "impossible_note", "") or "≠" in reaction.equation)
                db.add(JobReaction(
                    job_id=job_id,
                    ...
                    internet_status="does_not_react" if impossible else "not_checked",
                    internet_note=getattr(reaction, "impossible_note", "") or "",
                    review_reason="does_not_react" if impossible else "",
                    selected=not impossible,
                ))
